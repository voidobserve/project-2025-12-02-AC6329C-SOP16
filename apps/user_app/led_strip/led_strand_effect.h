...
enum
{
    THE_FASTEST_DYNAMIC_MODE_SPEED = 10,
    THE_SLOWEST_DYNAMIC_MODE_SPEED = 500,
};
...
typedef enum
{
    ...
    MODE_SEG_FLASH_RANDOM =
        22,                  //从的颜色池抽取颜色闪现，以段为单位，闪现位置随机
...
typedef struct
{
    ...
    unsigned char
        mode_cycle; //1:模式完成一个循环。0：正在跑，和meteor_period搭配用
...
void effect_smear_adjust_updata(smear_tool_e tool, hsv_t *colour,
                                unsigned short *led_place);
