...
u8 get_sound_result(void)
{
    if (fc_effect.sound.valid == 1) {
        ...
        if (fc_effect.sound.c_v > fc_effect.sound.v) {
            if ((fc_effect.sound.c_v - fc_effect.sound.v) * 100 /
                    fc_effect.sound.v >
                fc_effect.sound.sensitive) //很灵敏
...
void led_pwm_init(void)
{
    ...
    if (gpio_read(IO_PORTB_06)) {
        ...
        extern void timer_pwm_init(JL_TIMER_TypeDef * JL_TIMERx, u32 pwm_io,
                                   u32 fre, u32 duty);
        timer_pwm_init(JL_TIMER0, IO_PORTB_06, 1000,
                       0); //调timer做pwm  通道号 R c

        is_rgbw = 1; // 纯白灯
    }
}
 
static volatile u32 syn_ms = 0;
volatile u32 syn_edge_cnt = 0; // 已收到的过零边沿数(调试用)
...
void io_ext_interrupt_syn(void)
{
    ...
    // DP(USB0_DP)当作普通IO使用：本工程未使用USB功能(TCFG_OTG_USB_DEV_EN = 0)，
    // 参考 cpu/bd19/iic_hw.c 用 DP/DM 做 IIC 时的做法
    usb_iomode(1);
...
void fc_driver(u8 r, u8 g, u8 b)
{
    ...
    if (fc_effect.w == 0) {
        ...
        if (is_rgbw)
            set_timer_pwm_duty(JL_TIMER0,
                               fc_effect.w * 10000 / 255 * fc_effect.b / 255);
    }
}
