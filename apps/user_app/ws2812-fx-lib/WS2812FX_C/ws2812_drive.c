...
void ws281x_show(unsigned char *pixels_pattern, unsigned short pattern_size)
{
    ...
    fc_driver(*pixels_pattern, *(pixels_pattern + 1), *(pixels_pattern + 2));

    //该数据处理是仅有白光的流星使用
    unsigned short i, j = 0, k = 0;
    u8 r, g, b;
    for (i = 0; i < pattern_size / 3; i++) {
        *(buf + i) = *(pixels_pattern + 3 +
                       i * 3); //sjf  +3是因为前面有一个灯 RGB，地址不是从零开始
    }
    for (i = 0; i < pattern_size / 3; i++) {
        if (j == 0) {
            r = *(buf + i);
    ...
    extern void ledc_send_rgbbuf_isr(u8 index, u8 * rgbbuf, u32 buf_len,
                                     u16 again_cnt);
